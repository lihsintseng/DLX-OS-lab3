...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    freemem = MemoryAllocPage();
    pcbs[i].pagetable[0] = MemorySetupPte(freemem);
...
//  printf("Cleanup current PID: %d\n", currentPCB - pcbs);
  //------------------------------------------------------------
  // STUDENT: Free any memory resources on process death here.
  //------------------------------------------------------------
  addr = (uint32)pcb->sysStackPtr & (~MEM_ADDRESS_OFFSET_MASK);
  addr >>=  MEM_L1FIELD_FIRST_BITNUM;
  free = addr;
  refcount[addr] --;
  free32 = free >> 5;
  free = free % 32;
...
//  printf("Page 0: %x %d\n", pcb->pagetable[0], refcount[pcb->pagetable[0] >>MEM_L1FIELD_FIRST_BITNUM ]);
  for(i = 0 ; i < 256; i++){
...
int ProcessRealFork(){
  ...
  unsigned char * parentstack;
  unsigned char * childstack;
  ...
  //printf("Going into fork\n");
  tempaddr = MemoryAllocPage();
  stackframe = ((tempaddr<< MEM_L1FIELD_FIRST_BITNUM) + MEM_PAGESIZE - 1) & (~0x3);
  child->sysStackPtr = stackframe;
  //child->currentSavedFrame = (uint32)child->currentSavedFrame + (uint32)stackframe - (uint32)parent->sysStackPtr;
  //child->currentSavedFrame = ;
  //child->currentSavedFrame = (parent->currentSavedFrame) - parent->sysStackPtr + stackframe;
  //child->currentSavedFrame[PROCESS_STACK_PTSIZE] = (MEM_MAX_VIRTUAL_ADDRESS+1) >> MEM_L1FIELD_FIRST_BITNUM;
  //child->currentSavedFrame[PROCESS_STACK_PTBITS] = MEM_L1FIELD_FIRST_BITNUM + (MEM_L1FIELD_FIRST_BITNUM << 16);
  //printf("Parent: %x\t%x\t%x\t%x\n", parent->sysStackPtr,parent->currentSavedFrame, parent->l, parent->pagetable);

  //bytesToCopy = MEM_PAGESIZE - (parent->currentSavedFrame - parent->sysStackPtr); 
  parentstack = (uint32)parent->sysStackPtr & (~MEM_ADDRESS_OFFSET_MASK);
  childstack = (uint32)child->sysStackPtr & (~MEM_ADDRESS_OFFSET_MASK);
  //printf("%d pcb stackframe refcount: %d\n", child-pcbs, refcount[((uint32)(childstack) >> MEM_L1FIELD_FIRST_BITNUM)]);
   
  child->currentSavedFrame = (uint32)child->currentSavedFrame + childstack-parentstack;
  bcopy(parentstack, childstack, MEM_PAGESIZE);
  
  child->currentSavedFrame[PROCESS_STACK_PTBASE] = child->pagetable;
    //stackframe[PROCESS_STACK_USER_STACKPOINTER] = (MEM_MAX_VIRTUAL_ADDRESS) & (~0x3);
  //child->currentSavedFrame[PROCESS_STACK_USER_STACKPOINTER]  = (MEM_MAX_VIRTUAL_ADDRESS) & (~0x3);

  for(i = 0; i <= MEM_MAX_VIRTUAL_ADDRESS >> MEM_L1FIELD_FIRST_BITNUM; i++){
    pte = child->pagetable[i];
    valid = (pte & MEM_PTE_VALID) == 0 ? 0 : 1;
    if(valid != 0){
      //printf("addr: %d: refcount: %d\n", addr, refcount[addr]);
      addr = pte >> MEM_L1FIELD_FIRST_BITNUM;
      refcount[addr]++;
  ...
  if ((child->l = AQueueAllocLink(child)) == NULL) {
    printf("FATAL ERROR: could not get link for forked child in ProcessRealFork!\n");
    exitsim();
  }
  //printf("Child: %x\t%x\t%x\t%x\n", child->sysStackPtr,child->currentSavedFrame, child->l, child->pagetable);
//  if (AQueueInsertAfter(&runQueue, parent->l, child->l) != QUEUE_SUCCESS) {
  if (AQueueInsertFirst(&runQueue, child->l) != QUEUE_SUCCESS) {
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  // System stack 0000~0FFF
  // User stack 1000~1FFF
  // Assign 4 pages 2000~2FFF 3000~3FFF 4000~4FFF 5000~5FFF
//  stackframe = (MEM_PAGESIZE -1 ) & (~0x3);//0x6FFC; // 6000 ~ 6FFF
  stackframe = ((MemoryAllocPage()<< MEM_L1FIELD_FIRST_BITNUM) + MEM_PAGESIZE - 1) & (~0x3);//0x6FFC; // 6000 ~ 6FFF
 // printf("Stackframe, %d\n", stackframe);
  ...
  //printf("New process : %d %x\n", pcb-pcbs, pcb->sysStackPtr);
  ...
  // stack frame.
  //----------------------------------------------------------------------
   stackframe[PROCESS_STACK_PTBASE] = pcb->pagetable;//(uint32)&(pcb->pagetable[0]);
   stackframe[PROCESS_STACK_PTSIZE] = (MEM_MAX_VIRTUAL_ADDRESS+1) >> MEM_L1FIELD_FIRST_BITNUM;
...
void ProcessPrintPages(){
  uint32 pte;
  int i;
  printf("Pid: %d Print the valid pages:\n", currentPCB - pcbs);
  for(i = 0; i <= (MEM_MAX_VIRTUAL_ADDRESS >> MEM_L1FIELD_FIRST_BITNUM); i++){
    pte = currentPCB->pagetable[i];
    if(pte & MEM_PTE_VALID != 0){
      printf("PTE table[%d]:\t%x\n", i, pte);
    }
  }
}
