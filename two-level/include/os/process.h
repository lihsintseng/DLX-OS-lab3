...
typedef struct PCB {
  ...
  //uint32	pagetable[256/* Put the size of the L1 page table here */]; // Statically allocated page table
  uint32	pagetable[4/* Put the size of the L1 page table here */]; // Statically allocated page table
  uint32    L2pagetable[4][64];
  Link		*l;		// Used for keeping PCB in queues
} PCB;

extern PCB	*currentPCB;
extern uint32 freemap[512/32]; // PhyMem: 2M, Pagesize: 4K
