...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    pcbs[i].pagetable[0] = pcbs[i].L2pagetable[0];
    pcbs[i].pagetable[1] = pcbs[i].L2pagetable[1];
    pcbs[i].pagetable[2] = pcbs[i].L2pagetable[2];
    pcbs[i].pagetable[3] = pcbs[i].L2pagetable[3];
    pcbs[i].L2pagetable[0][0] = MemorySetupPte(freemem);
    //pcbs[i].L2pagetable[1][0] = MemorySetupPte(freemem);
    //pcbs[i].L2pagetable[2][0] = MemorySetupPte(freemem);
    //pcbs[i].L2pagetable[3][0] = MemorySetupPte(freemem);
...
void ProcessFreeResources (PCB *pcb) {
  ...
  addr >>=  MEM_L2FIELD_FIRST_BITNUM;
  free = addr;
  free32 = free >> 5;
  free = free % 32;
  freemap[free32] |= (1 << free);
  for(j = 0; j < 4 ; j ++){
    l2addr = pcb->pagetable[j];
    for(i = 0 ; i < (1<<(MEM_L1FIELD_FIRST_BITNUM - MEM_L2FIELD_FIRST_BITNUM)); i++){
      MemoryFreePage(l2addr[i]);
      l2addr[i]  &= ~(MEM_PTE_VALID);
    }
  }
...
//  stackframe = (MEM_PAGESIZE -1 ) & (~0x3);//0x6FFC; // 6000 ~ 6FFF
  stackframe = ((MemoryAllocPage()<< MEM_L2FIELD_FIRST_BITNUM) + MEM_PAGESIZE - 1) & (~0x3);//0x6FFC; // 6000 ~ 6FFF
  ...
  ((uint32*) pcb->pagetable[0])[0] = MemorySetupPte(MemoryAllocPage());
  ((uint32*) pcb->pagetable[0])[1] = MemorySetupPte(MemoryAllocPage());
  ((uint32*) pcb->pagetable[0])[2] = MemorySetupPte(MemoryAllocPage());
  ((uint32*) pcb->pagetable[0])[3] = MemorySetupPte(MemoryAllocPage());
  ...
  //----------------------------------------------------------------------
   ...
   stackframe[PROCESS_STACK_PTSIZE] = 4;//(MEM_MAX_VIRTUAL_ADDRESS+1) >> MEM_L2FIELD_FIRST_BITNUM;
  ...
  if (isUser) {
    ...
    //pcb->pagetable[MEM_MAX_VIRTUAL_ADDRESS >> MEM_L2FIELD_FIRST_BITNUM ] = MemorySetupPte(MemoryAllocPage());    
    l2addr = pcb->pagetable[3];
    l2addr[(1<<(MEM_L1FIELD_FIRST_BITNUM - MEM_L2FIELD_FIRST_BITNUM))-1] = MemorySetupPte(MemoryAllocPage()); 
